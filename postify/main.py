...
class Order:
    ...
    def get_order(self, identification: str):
        ...
        status = None
        sh_inst = Shopify(order_id=identification)
        try:
            ...
            if scheduled_order:
                ...
                order_response.update({"Scheduled on ": scheduled_order.Entry_Date})
                # Order Status
                if scheduled_order.is_despatched_and_bagged() == True:
                    ...
                    order_response["Status"] = (
                        "Scheduled, Tracking link will be available soon."
                    )
            else:
                ...
                if unscheduled_order:
                    ...
                    order_response["Order date"] = unscheduled_order["createdAt"].split(
                        "T"
                    )[0]
                    order_response["Status"] = (
                        f"Your order has been successfully confirmed and will be dispatched on the next working day. The estimated delivery time is 3–5 business days via India Post Speed Post."
                    )
                else:
                    status = 404
    ...
    def order_page(self, request: Request, identification: str):
        track = Tracking()
        try:
            return track.show_tracking_page(
                request=request, identification=identification
            )
        except Exception as e:
...
class Pickup:
    ...
    def missing_form(self, request: Request):
        ...
        context = {"timestamps": reversed([timestamp[0] for timestamp in entry_dates])}
        try:
...
class Tracking:
    ...
    def track_order(self, request: Request, order_id: str = Form()):
        tracked_order = None
        try:
            tracked_order = self.show_tracking_page(
                request=request, identification=order_id
            )
        except Exception as e:
            print(e)
...
router.add_api_route(
    order.base_url + "{identification}", order.get_order, methods=["GET"]
)
router.add_api_route(
    order.base_url + "{identification}/html", order.order_page, methods=["GET"]
)

pickup = Pickup()
router.add_api_route(
    pickup.base_url + "missing_form", pickup.missing_form, methods=["GET"]
)
router.add_api_route(
    pickup.base_url + "find_missing", pickup.find_missing_orders, methods=["POST"]
)
...
router.add_api_route(
    tracking.base_url + "/{identification}",
    tracking.show_tracking_page,
    methods=["GET"],
)
...
app.mount(
    "/postify/static", StaticFiles(directory="postify/static"), name="postify_static"
)
